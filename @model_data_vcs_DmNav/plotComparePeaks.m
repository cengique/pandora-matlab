function a_p = plotComparePeaks(a_md, title_str, props)

% plotComparePeaks - Plot I/V curves comparing model and data.
%
% Usage:
% a_p = plotComparePeaks(a_md, title_str, props)
%
% Parameters:
%   a_md: A model_data_vcs_DmNav object.
%   title_str: (Optional) Text to appear in the plot title.
%   props: A structure with any optional properties.
%     quiet: If 1, only use given title_str.
%		
% Returns:
%   a_p: A plot_abstract object.
%
% Description:
%
% Example:
% >> a_md = model_data_vcs_DmNav(model, data_vc)
% >> plotFigure(plotComparePeaks(a_md, 'I/V curves'))
%
% See also: model_data_vcs_DmNav, voltage_clamp, plot_abstract, plotFigure
%
% $Id: plotComparePeaks.m 89 2010-04-09 19:54:00Z cengiz $
%
% Author: Cengiz Gunay <cgunay@emory.edu>, 2010/10/11

if ~ exist('props', 'var')
  props = struct;
end

if ~ exist('title_str', 'var')
  title_str = '';
end

if isfield(props, 'quiet')
  all_title = title_str;
else
  all_title = [' IV act, steady, inact; data vs model' title_str ];
end

% plot comparison of data and model peaks 
a_p = ...
    plot_superpose({...
      plotSteadyIV(calcCurPeaks(a_md.data_vc, 2, struct('pulseEndRel', [1 10])), 2, ...
                   all_title, ...
                   struct('noTitle', 1, ...
                          'grid', 1, 'label', 'act', 'plotPeaks', 1, ...
                          'plotProps', ...
                          struct('Marker', '+', 'LineStyle', 'none', 'Color', 'b'))), ...
      plotSteadyIV(calcCurPeaks(a_md.model_vc, 2, ...
                                struct('pulseStartRel', [1 .1], 'pulseEndRel', [1 10])), 2, '', ...
                   struct('noTitle', 1, ...
                          'label', 'act (model)', 'plotPeaks', 1, ...
                          'plotProps', ...
                          struct('Color', 'b'))), ... 
      plotSteadyIV(calcCurPeaks(a_md.data_vc, 2, struct('pulseStartRel', [2 -30])), 2, '', ...
                   struct('noTitle', 1, ...
                          'grid', 1, 'label', 'act-end', 'plotPeaks', 1, ...
                          'plotProps', ...
                          struct('Marker', 'o', 'LineStyle', 'none', 'Color', 'r'))), ...
      plotSteadyIV(calcCurPeaks(a_md.model_vc, 2, struct('pulseStartRel', [2 -30])), 2, '', ...
                   struct('noTitle', 1, ...
                          'label', 'act-end (model)', 'plotPeaks', 1, ...
                          'plotProps', ...
                          struct('Color', 'r'))), ...
      plotSteadyIV(calcCurPeaks(a_md.data_vc, 3, ...
                                struct('pulseStartRel', [2 .5], 'pulseEndRel', [2 10])), 2, '', ...
                   struct('noTitle', 1, ...
                          'grid', 1, 'label', 'inact', ...
                          'plotPeaks', 1, ...
                          'plotProps', ...
                          struct('Marker', 'x', 'LineStyle', 'none', 'Color', 'g'))), ...
      plotSteadyIV(calcCurPeaks(a_md.model_vc, 3, ...
                                struct('pulseStartRel', [2 .1], 'pulseEndRel', [2 10])), 2, '', ...
                   struct('noTitle', 1, ...
                          'label', 'inact (model)', ...
                          'plotPeaks', 1, ...
                          'plotProps', ...
                          struct('Color', 'g')))}, ...
                   {}, '', ...
                   struct('noCombine', 1, ...
                          'fixedSize', [2.5 2]));
