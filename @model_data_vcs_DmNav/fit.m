function [a_md sub_vc] = fit(a_md, title_str, props)

% fit - Fit model to data.
%
% Usage:
% [a_md sub_vc] = fit(a_md, title_str, props)
%
% Parameters:
...
%   sub_vc: voltage_clamp object with subtracted I trace showing fitting errors.
%
% Description:
%
% Example:
...
%
% $Id: fit.m 172 2010-10-06 00:38:29Z cengiz $
%
% Author: Cengiz Gunay <cgunay@emory.edu>, 2010/10/12

% TODO: 
% - separate plotting to other methods
...
plotFigure(...
  ...
  % choose the range
  period_range = getFieldDefault(props, 'period', periodWhole(data_vc));
% $$$   period_range = period(round(data_vc.time_steps(1) - 10 / dt), ...
% $$$                         round(data_vc.time_steps(2) + 30 / dt));

  % restrict the data_vc to given range to prepare for subtraction
  [a_range_vc period_range] = ...
      withinPeriod(data_vc, period_range, struct('useAvailable', 1)); 

  range_steps = array(period_range);

  model_vc = simModel(a_range_vc, f_model);
  ...
  plotFigure(...
    plot_stack({...
      plot_superpose({...
        plot_abstract({time, data_vc.i.data}, {'time [ms]', 'I_{data}&I_{sim} [nA]'}, ...
                      'data', v_legend, 'plot', ...
                      struct('ColorOrder', line_colors)), ...
