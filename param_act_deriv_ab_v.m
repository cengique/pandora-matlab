...
% TODO:
% - this should be class with conversion functions to get Inf and Tau
% - allow setting initial value rather than taking it from v(1)
  ...
  % returns an param_act function
    p = getParamsStruct(a_pf);
    p_inf = param_act([-p.k*log(p.b0/p.a0) p.k], 'inf');
  end

  function p_tau_m = getTau(a_pf)
  % returns an param_act function
    p = getParamsStruct(a_pf);
    p_tau_m = ...
        param_tau_skewbell_v(struct('tau0', 1/(p.a0^(1-p.delta)* ...
                                               p.b0^(p.delta)), ...
                                    'delta', p.delta, ...
