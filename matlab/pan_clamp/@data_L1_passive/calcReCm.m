...
if isfield(props, 'ifPlot') || verbose
  ...
  plotFigure(...
    plot_abstract({(start_dt:end_dt)*dt, Icap, ...
                   (start_dt + timeconstant) * dt, Icap(timeconstant), '*r', ...
                  (start_dt:end_dt)*dt, Iideal, '--g', ...
                   (start_dt:end_dt)*dt, [Icap(2:end); Icap(end)] - Iideal(:), '-.k'}, ...
                  {'time [ms]', ['I [nA] / ' num2str(nA_scale)]}, '', ...
                  {'recorded', 't=\tau', 'exp(-t/\tau)', 'sub'}, ...
                  'plot', ...
                  struct('axisLimits', ...
