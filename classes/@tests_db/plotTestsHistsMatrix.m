...
% $$$     end
    ...
    all_pm(test_num) = plot_stack(plots, axis_limits, 'y', all_title, ...
				  extra_props);
  end
  % final horizontal stack
  if num_tests > 1
      all_pm = num2cell(all_pm)
      a_pm = plot_stack(all_pm, [], 'x', ...
                        all_title, mergeStructs(props, ...
                                                struct('xLabelsPos', 'bottom', 'xTicksPos', 'bottom', ...
                                                       'yLabelsPos', 'left')));
  else
      % no stack needed if only one vertical stack
      a_pm = all_pm;
      a_pm.props = mergeStructs(props, a_pm.props);
  end
else % num_dbs > 1
