...
elseif isa(data_src,'trace') % copy constructor?
    ...
    if isa(data_src, 'char') % filename?
        ...
        if strcmpi(props.file_type, 'cbin') % Added this for handling .cbin files
            if isfield(props, 'channel')
                channel = props.channel;
            else
                channel = 'obs0'; % Default
            end
            [data,fs] = evsoundin(path, [filename, ext], channel);
        elseif strcmpi(props.file_type, 'genesis')
